...
#define MAX_VALUES 8 // Number of values used in aggregation process (K)
...
/*----------------------- code demo ----------------------------------------*/
void EMAsmoothing(float input[]){
    int i=0;
    input[i] = gamma * input[i] + (1-gamma) * input[i];   
    for(i=1; i<BUFFER_SIZE; i++){
	input[i] = gamma * input[i] + (1-gamma) * input[i-1];
    }
}
float calculate_var(float arr[]) {
    int n = BUFFER_SIZE;
    float mean = calculate_mean(arr,n);
    float sum_squared_diff = 0;
    int i;
...
PROCESS_THREAD(read_process, ev, data)
{
    ...
    while (1)
    {
        ...
        if (count >= MAX_VALUES && buffer[BUFFER_SIZE-1] > 0)
        {
...
PROCESS_THREAD(m_process, ev, data)
{
    ...
    int segments, k, l;
    static float cutoffRange[4] = {(-0.84), (-0.25), 0.25, 0.84};
	...
	int i;
	for(i=0; i<segments; i++){
	    if (aggData[i] < cutoffRange[0] && isNegative(aggData[i])){
		sax[i] = 'A';
	    }
	    else if (aggData[i] < cutoffRange[1] && isNegative(aggData[i])){
		sax[i] = 'B';
	    }
	    else if (aggData[i] < cutoffRange[2]){
		sax[i] = 'C';
	    }
	    else if (aggData[i] < cutoffRange[3]){
		sax[i] = 'D';
	    }
	    else{
		sax[i] = 'E';
